...
extension Frame {
    ...
    var view: AnyView_ {
        ...
        case .fixedWidth(let width):
            return AnyView_(r.frame(width: width))
        case .min(let width):
            return AnyView_(r.frame(minWidth: width))
        case .max(let width):
            return AnyView_(r.frame(maxWidth: width))
...
class SwiftUILayoutiOSTests: XCTestCase {
    ...
    func testExample() throws {
        for _ in 0..<100 {
            ...
            let subviews = frames.map(\.view)
            let swiftUIViews = subviews.map(\.swiftUI)
            ...
            XCTAssertEqual(swiftUISizes, stack.sizes.map(\.width), "\(frames) - proposedWidth: \(proposedWidth )")
        }
    }
}
